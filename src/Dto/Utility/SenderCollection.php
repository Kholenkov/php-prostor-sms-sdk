 ...
 *
 * @implements Iterator<int, Sender>
 */
final class SenderCollection implements Countable, Iterator, JsonSerializable
{
    private int $key;
    private array $values;

    public function __construct(Sender ...$values)
    {
        $this->key = 0;
        $this->values = $values;
    }
    ...
    public function current(): Sender
    {
    ...
    public static function fromArray(array $array, string $keyForException = ''): self
    {
        ...
        foreach ($array as $i => $value) {
            ...
            $values[] = Sender::fromArray($value, "{$keyForException}[{$i}]");
        }
